...
import { createResultsList, createDenseResultsList } from "../../target/elements.js";
...
function handleFilter() {
  window.addEventListener("DOMContentLoaded", async () => {
    document.getElementById("display-style").addEventListener("change", (e) => {
      ...
      window.api.filterOptions(filterOptions).then((appearances) => displayElements(filterOptions, appearances));
    });
  });
}
...
export function displayElements(opt, elements, charName) {
  ...
  const appearances = elements.map((element) => {
    ...
    return new ListEntry(element.title, date.year, date.month, date.day, element.links);
  });
  ...
  if (charName) {
    document.getElementById("results-header").textContent = `${charName} Appearances`;
  }
}
